function solve() {
    ...
    function creator(button) {
        ...
        if(button.target.textContent == "Add" && task.value != "" && description.value != "" && date.value != "") {
            ...
            Array.from(allSections[1].querySelectorAll("div"))[1].appendChild(article);
        }
        if(button.target.textContent == "Start") {
            ...
            Array.from(allSections[2].querySelectorAll("div"))[1].appendChild(information);
        }
        else if(button.target.textContent == "Finish") {
            information.remove();
            information.querySelector("div").remove();
            Array.from(allSections[3].querySelectorAll("div"))[1].appendChild(information);
        }
        else if(button.target.textContent == "Delete") {
            information.remove();
        }
        Array.from(document.querySelectorAll("button")).forEach(btn => btn.addEventListener("click", creator));
